...
import requests
import logging
import time  # <-- AÑADIDO para los reintentos
from functools import wraps  # <-- AÑADIDO para el decorador
...
# --- ¡NUEVO! Decorador para Reintentos Automáticos ---
def retry_on_failure(retries=3, delay=2, backoff=2):
    """
    Un decorador que reintenta una función si esta lanza una excepción.
    :param retries: Número máximo de intentos.
    :param delay: Tiempo de espera inicial en segundos.
    :param backoff: Multiplicador para el tiempo de espera en cada reintento (backoff exponencial).
    """
    def decorator(func):
        @wraps(func)
            ...
            for i in range(retries):
                try:
                    ...
                    if i == retries - 1:
                        ...
                        raise e  # Lanza la última excepción si todos los reintentos fallan
...
def upload_to_gcs(file_stream, destination_blob_name, content_type='audio/mpeg'):
    ...
    try:
        ...
        logging.error(f"Error al subir el archivo '{destination_blob_name}' a GCS.", exc_info=True)
        # Se relanza la excepción para que el decorador de reintentos la capture
        raise
...
def safe_json_parse(text):
    ...
    try:
        ...
        logging.error(f"Error al decodificar JSON. Texto problemático: {text}", exc_info=True)
        return None


# --- ¡NUEVO! Lógica de generación de audio separada para aplicar reintentos ---
...
def _generate_audio_with_api(script, voice_id):
    ...
    El decorador @retry_on_failure se encargará de los reintentos.
    """
    ...
    # Si la subida falla, upload_to_gcs ahora lanza una excepción que será capturada por el decorador
    return public_url
...
@app.route("/")
def index():
    return "Backend de IA para Videos - ¡Corriendo con reintentos automáticos!"

# ... (El resto de los endpoints como generate_initial_content y regenerate_scene_part no se modifican) ...
...
def regenerate_scene_part():
    ...
    if not scene or not part:
        logging.warning("Llamada a regenerate-scene-part sin 'scene' o 'part'.")
    ...
    if part == 'script':
        try:
            ...
            logging.info(f"Script regenerado con éxito para escena ID: {scene.get('id')}")
        ...
        try:
            ...
            logging.info(f"Generando imagen con Vertex AI. Prompt: '{image_prompt}', Aspect Ratio: {aspect_ratio}")
            ...
            logging.info(f"Imagen generada en URL temporal. Descargando...")
            ...
            logging.info(f"Media regenerada con éxito para escena ID: {scene.get('id')}")
    ...
    logging.warning(f"Se intentó regenerar una parte no válida: '{part}'")
    return jsonify({"error": "Parte no válida para regenerar"}), 400


@app.route('/api/generate-and-save-audio', methods=['POST'])
def generate_audio():
    # --- ¡MEJORADO! Este endpoint ahora usa la función con reintentos ---
    data = request.get_json()
    script = data.get('script')
    voice_id = data.get('voice', 'es-US-Wavenet-B')

    if not script:
        logging.warning("Llamada a generate_audio sin 'script'.")
    ...
    try:
        # Llamamos a la nueva función que tiene la lógica de reintentos
        public_url = _generate_audio_with_api(script, voice_id)
        
        logging.info(f"Proceso de generación de audio completado exitosamente. URL: {public_url}")
        return jsonify({"audioUrl": public_url})

    except Exception as e:
        # Este bloque ahora se ejecuta solo si todos los reintentos fallan
        logging.error("FALLO CRÍTICO en la generación de audio después de todos los reintentos.", exc_info=True)
        return jsonify({"error": f"No se pudo generar el audio después de varios intentos: {str(e)}"}), 500
...
# ... (El endpoint generate_seo no se modifica) ...
...
def generate_seo():
    try:
        ...
        if not guion:
            logging.warning("Llamada a generate_seo sin 'guion'.")
        ...
        if parsed_json:
            logging.info("Contenido SEO generado y parseado con éxito.")
            return jsonify(parsed_json)
        else:
            logging.error("La respuesta del modelo de SEO no tuvo el formato esperado.")
